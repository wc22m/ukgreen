...
public class csvfilereader
{
    ...
    public  String[] getcolheaders( pcsv p)
    {
        ...
        if( p== null )
            ...
            {
                if(colheaders.get(p)==null)
                    {
                        h= new String[colcount(p)];
                        for (int i=0; i<h.length; i++)h[i]="";
                        recursegch (0,p .right,h);
                        colheaders.put(p,h);
                        return h;
                    }
            }
        return colheaders.get(p);
    }
    void recursegrh(int i, pcsv q, String[]h)
    {
        if( q != null )
            {
                if(q.right !=null)h [i] =(""+ q .right.tag).trim() ;
    ...
    public String[] getrowheaders(pcsv p )
    {
        ...
        if( p== null )
            ...
            {
                if(rowheaders.get(p)==null)
                    {
                        h= new String[ rowcount(p)];
                        for ( i=0; i<h.length; i++)h[i]="";
                        recursegrh (0,p,h);
                        rowheaders.put(p,h);
                        return h;
                    }
    ...
    public pcsv getline(pcsv p,int row  )
    {
        if( p== null )
            ...
            {
                return getline(((linestart)(p.tag)) .down,row-1 );
            }
    };
    ...
    token thetoken()
    {
        if( currentchar < bp.length )
            return categorisor[127&bp [currentchar]];
    ...
    void resolvealpha()
    {
        int i,l;
        {
            ...
            for( i = tokstart ; i<=( l -1); i++)
                {
                    p.textual = p.textual + ((bp [i]));
                };
            p.textual=p.textual.toUpperCase().trim();
        };
    };
    ...
    void resolvetoken()
    {
        if(chr(bp[tokstart])=='+'  ||chr(bp[tokstart])=='-'||( chr(bp [tokstart]) >='0' &&chr(bp [tokstart])<= '9' )) resolvedigits();
        else resolvealpha();
    };
    ...
    {
        parsedelimitedfields = parsedelimitedfields+(bp [currentchar]);
        nextsymbol();
    };
