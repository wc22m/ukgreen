        ...
        if (args.length<3) {
            ...
            for(int i=2; i<=rows; i++) {
                ...
                if(cell!=null)if(head!=null) {
                        ...
                        headernum.put(transcol0[i-1],new Integer(i));
            ...
            for(int i=1; i<validcols; i++) {
                ...
                if(! match .equals("NOT IN TABLE")) { // this is a valid header
                    ...
                    int[] positions = new int[all.size()];
                    for(int j=0; j<all.size(); j++)positions[j]=(int)headernum.get(all.elementAt(j));
                    // sum the total output of this class of product
                    double tot=0;
                    for(int k:positions) {
                        //	     System.out. print("pos "+k+" "+outputvector[k]);
                        tot+=outputvector[k];
                    }

                    double frac = outputvector[i]/tot;// the fraction of the capital stock
                    // of the given class to go under this header
                    //System.out.println(outputheaders[i]+"/"+match+"="+frac);
